...
import { BookA, NotebookPen, Trash, Trash2 } from "lucide-react";
...
const BookManagement = () => {
  ...
  const { addBookPopup, readBookPopup, recordBookPopup, deleteBookPopup, bookIdToDelete } 
            = useSelector((state)=> state.popup);

  const {loading: borrowSliceLoading, error: borrowSliceError, 
    ...
    <header className="flex flex-col gap-3 md:flex-row md:justify-between md:items-center">
      <h2 className="text-xl font-medium md:text-2xl md:font-semibold">
        ...
        <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4">
          ...
          overflow-hidden rounded-full text-black w-[25px] h-[25px] text-[27px] absolute left-5">
            +
          </span>
                Add Book</button>
            )}
            <input type="text" placeholder="Search books..." className="w-full sm:w-52 border p-2 
            border-gray-300 rounded-md" value={searchedKeyword} onChange={handleSearch}/>
        </div>
    </header>
